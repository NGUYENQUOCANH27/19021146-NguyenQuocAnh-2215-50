...
enum GameState { MENU, PLAY, GAMEOVER, HELP, HIGHSCORE, CREDITS };
...
private:
    ...
    Mix_Chunk* flapSound;  // Âm thanh “flap”
    TTF_Font* font;
...
bool FlappyBird::init() {
    ...
    flapSound = Mix_LoadWAV("assets/flap.wav");  // Tải âm thanh “flap”
            ...
            switch (e.key.keysym.sym) {
                case SDLK_SPACE:
                    if (gameState == PLAY) {
                        ...
                        Mix_PlayChannel(-1, flapSound, 0);  // Phát âm thanh “flap”
                ...
                case SDLK_h:
                    ...
                    break;
                case SDLK_m:
                    if (gameState == HELP || gameState == HIGHSCORE || gameState == CREDITS) {
                        gameState = MENU;
                    }
                    break;
                case SDLK_s:
...
void FlappyBird::render() {
    ...
    if (gameState == MENU) {
        ...
        renderText("Help:", SCREEN_WIDTH / 2 - 50, SCREEN_HEIGHT / 2 - 100, {64, 64, 64}); // Màu xám đậm
        renderText("Press SPACE to Flap", SCREEN_WIDTH / 2 - 150, SCREEN_HEIGHT / 2 - 50, {64, 64, 64}); // Màu xám đậm
        renderText("Avoid the Pipes!", SCREEN_WIDTH / 2 - 120, SCREEN_HEIGHT / 2, {64, 64, 64}); // Màu xám đậm
...
void FlappyBird::renderCredits() {
    ...
    renderText("Created by: Quocanh19021146", SCREEN_WIDTH / 2 - 150, SCREEN_HEIGHT / 2 - 50, {64, 64, 64}); // Màu xám đậm
...
void FlappyBird::close() {
    ...
    Mix_FreeChunk(flapSound);  // Giải phóng âm thanh “flap”
    TTF_CloseFont(font);
